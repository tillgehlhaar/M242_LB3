...
// Topic's publish
char* topicTEMP = (char*) "sansiv/sensor";
char* topicALERT = (char*) "sansiv/alert";
char* topicENCODER = (char*) "sansiv/encoder";
// Topic's subscribe
char* topicActors = (char*) "sansiv/actors/#";
// MQTT Brocker
char* hostname = (char*) "192.168.1.144";
int port = 1883;
// MQTT Message
...
int main()
{
    ...
    while   ( 1 ) 
    {
        ...
        sprintf( buf, "%4.4f", hallSensor.read()); 
        sprintf( buf, "%4.4f", hallSensor.read()); 
        type++;
        if  ( type > 2 )
            type = 0;       
